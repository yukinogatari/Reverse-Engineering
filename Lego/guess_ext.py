...
def guess_ext(data, platform = None):
  ...
  exts = [
    # fib v1 audio
    (b"\x01\x00\x00\x00\x2C\x00\x00\x00\x2C\x00\x00\x00", 0, ".lwav"),
    # fib v2 audio
    (b"\xF2\xFF\xFF\xFF\x2C\x00\x00\x00", 8, ".lwav"),
    
    # fib v1 3DS graphics / overlaps with fib v1 PSP audio
    (b"\x01\x00\x00\x00\x20\x00\x00\x00\x20\x00\x00\x00", 0, ".lwav" if platform == PLATFORM_PSP else ".lga"),
    # fib v2, LEGO Batman 3 3DS graphics
    (b"\x00\x04\x00\x00\x10\x00\x00\x00\xF2\xFF\xFF\xFF\x20\x00\x00\x00", 0, ".lga"),
    # fib v2, LEGO Movie Vita graphics
    (b"\x00\x04\x00\x00\x10\x00\x00\x00\xF2\xFF\xFF\xFF\x1C\x00\x00\x00", 0, ".lga"),
    
    # fib v1 PSP graphics
    (b"\x50\x00\x00\x00", 8, ".lga"),
    
    # Mostly guesses based on games w/ csv files
    (b"\x04\x00\x00\x00", 8, ".xaml"),
    (b"\x08\x00\x00\x00", 8, ".ltbl"), # converted xls files
    ...
    (b"\xF2\xFF\xFF\xFF\x04\x00\x00\x00", 8, ".xaml"),
    (b"\xF2\xFF\xFF\xFF\x08\x00\x00\x00", 8, ".ltbl"),
